...
async def async_setup_entry(
    ...
    def _async_add_new_sensors() -> None:
        """Create entities for any sensor keys not yet added to HA.

        A sensor key can be transiently absent from the coordinator's data on
        a single poll (e.g. signal strength reads "--" for a sensor with a
        weak RF link right after a restart). Entities used to be created only
        once, from whichever keys were present on the very first refresh, so
        a key missing at that moment never got an entity for the rest of the
        session even after the data reappeared. Re-scanning on every
        ...
        if new_entities:
            _LOGGER.info("Adding %d Ecowitt Local sensor entities", len(new_entities))
            async_add_entities(new_entities, True)

    _async_add_new_sensors()
    config_entry.async_on_unload(coordinator.async_add_listener(_async_add_new_sensors))
